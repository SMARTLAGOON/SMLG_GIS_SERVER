...
(L.Control.Coordinates = L.Control.extend({
    ...
    onAdd: function (a) {
        ...
        return (
            d.useLatLngOrder
                ? ((f = L.DomUtil.create("span", "", this._inputcontainer)),
                  (this._inputY = this._createInput("inputY", this._inputcontainer)),
                  (e = L.DomUtil.create("span", "", this._inputcontainer)),
                  (this._inputX = this._createInput("inputX", this._inputcontainer)))
                : ((e = L.DomUtil.create("span", "", this._inputcontainer)),
                  (this._inputX = this._createInput("inputX", this._inputcontainer)),
                  (f = L.DomUtil.create("span", "", this._inputcontainer)),
                  (this._inputY = this._createInput("inputY", this._inputcontainer))),
            (e.innerHTML = d.labelTemplateLng.replace("{x}", "")),
            (f.innerHTML = d.labelTemplateLat.replace("{y}", "")),
            L.DomEvent.on(this._inputX, "keyup", this._handleKeypress, this),
            L.DomEvent.on(this._inputY, "keyup", this._handleKeypress, this),
            a.on("mousemove", this._update, this),
            a.on("dragstart", this.collapse, this),
            a.whenReady(this._update, this),
            (this._showsCoordinates = !0),
            d.enableUserInput && L.DomEvent.addListener(this._container, "click", this._switchUI, this),
            c
        );
    },
    ...
    _handleSubmit: function () {
        var a = L.NumberFormatter.createValidNumber(this._inputX.value, this.options.decimalSeperator),
            ...
            this._map.panTo(new L.LatLng(b, a));
    ...
    _createCoordinateLabel: function (a) {
        var b,
            c,
            d = this.options;
        return d.customLabelFcn
            ? d.customLabelFcn(a, d)
            : ((b = d.labelFormatterLng ? d.labelFormatterLng(a.lng) : L.Util.template(d.labelTemplateLng, { x: this._getNumber(a.lng, d) })),
              (c = d.labelFormatterLat ? d.labelFormatterLat(a.lat) : L.Util.template(d.labelTemplateLat, { y: this._getNumber(a.lat, d) })),
    ...
    _update: function (a) {
        ...
        b &&
            ((b = b.wrap()),
            (this._currentPos = b),
            (this._inputY.value = L.NumberFormatter.round(b.lat, c.decimals, c.decimalSeperator)),
            (this._inputX.value = L.NumberFormatter.round(b.lng, c.decimals, c.decimalSeperator)),
            (this._label.innerHTML = this._createCoordinateLabel(b)));
    },
    ...
    L.Map.mergeOptions({ coordinateControl: !1 }),
